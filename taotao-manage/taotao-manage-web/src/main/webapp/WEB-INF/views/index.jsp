...
$(function(){
	...
	//tree函数来自easyUI的类库
	$('#menu').tree({
		onClick: function(node){ //node的被点击的节点 li
			if($('#menu').tree("isLeaf",node.target)){ //判断当前被点击的节点对象是否是叶子节点
				console.info(node);
				var tabs = $("#tabs");//右侧整个div对象
				var tab = tabs.tabs("getTab",node.text);//根据当前的节点名称获取右侧打开的窗体对象
				if(tab){//如果右侧已经存在当前节点的对应窗体，则选中
					tabs.tabs("select",node.text);
				}else{
					//如果右侧不存在当前节点的对应窗体，新打开一个窗体
					tabs.tabs('add',{
					    title:node.text,
